...
void toom_cook_4way_avx(__m256i* a1_avx,__m256i* b1_avx, uint64_t p_mod, __m256i* res_avx_output){
	...
	//__m256i w1_avx_t[2*small_len_avx],w2_avx_t[2*small_len_avx],w3_avx_t[2*small_len_avx],w4_avx_t[2*small_len_avx],w5_avx_t[2*small_len_avx],w6_avx_t[2*small_len_avx],w7_avx_t[2*small_len_avx];
	...
	//pol_mul_avx(a_avx, b_avx, w7_avx, p_mod, small_len, 0);
	...
	pol_mul_avx(a1_ph_avx, b1_ph_avx, w7_avx, p_mod, small_len, 0);
	//-----second multiplication
	pol_mul_avx(a2_ph_avx, b2_ph_avx, w5_avx, p_mod, small_len, 0);
	//-----Third multiplication
	pol_mul_avx(a3_ph_avx, b3_ph_avx, w6_avx, p_mod, small_len, 0);
	//-----Fourth multilication
	pol_mul_avx(a4_ph_avx, b4_ph_avx, w3_avx, p_mod, small_len, 0);	
	//-----Fifth Multiplication
	pol_mul_avx(a5_ph_avx, b5_ph_avx, w4_avx, p_mod, small_len, 0);	
	//-----Sixth Multiplication
	pol_mul_avx(a6_ph_avx, b6_ph_avx, w1_avx, p_mod, small_len, 0);	
	//-----Seventh Multiplication
	pol_mul_avx(a_avx, b_avx, w2_avx, p_mod, small_len, 0);
	//printf("w2=\n");
	//print_poly1(w2_avx, 2*64-1, p_mod);
	*/
	...
	/*
	uint16_t result_k1[128], result_k2[128];
	for(i=0;i<8;i++)	
	{
		_mm256_maskstore_epi64 (result_k1+ i*16, mask, w5_avx[i]);
		_mm256_maskstore_epi64 (result_k2+ i*16, mask, w5_avx_t[i]);
	}
	for(i=0; i<127; i++)
	if(result_k1[i] != result_k2[i]) printf("Err %d\n", i);
	*/
